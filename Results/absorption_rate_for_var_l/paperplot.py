...
bd_measure[0,:] = [250,25,0.25]
bd_measure[1,:] = [7.24730482e-01,1.12827586e+00,9.02457930e-01]
bd_measure[2,:] = [3.18952981e-02,4.04150783e-02,3.54952254e-02]
bd_measure[1,:] = bd_measure[1,:]/k_inf
bd_measure[2,:] = bd_measure[2,:]/k_inf
...
ax1.annotate('increasing $l$', xy=(1,1.02))
ax1.annotate('', xy=(10**0.5, 1.05),  xycoords='data',
...
ln2a = mp.plot(rsrate[`g_values[0]`][:,0],  rsrate[`g_values[0]`][:,1], 'r--', label = 'slow \nlimit')
ln2b = mp.plot(rsrate[`g_values[1]`][:,0],  rsrate[`g_values[1]`][:,1], 'r--')
ln2c = mp.plot(rsrate[`g_values[2]`][:,0],  rsrate[`g_values[2]`][:,1], 'r--')
ln3a = mp.plot(rlrate[`g_values[0]`][:,0],  rlrate[`g_values[0]`][:,1], 'b-.', label = 'fast \nlimit')
ln3b = mp.plot(rlrate[`g_values[1]`][:,0],  rlrate[`g_values[1]`][:,1], 'b-.')
ln3c = mp.plot(rlrate[`g_values[2]`][:,0],  rlrate[`g_values[2]`][:,1], 'b-.')
mp.plot(statepoints[0], statepoints[1], 'kx', label = 'state \npoints')
mp.errorbar(bd_measure[0], bd_measure[1],yerr = bd_measure[2])
...
ax2.annotate('B)', xy=(10**-1.7, 2.7))
ax2.annotate('increasing $l$', xy=(10**-1, 1.2))
ax2.annotate('', xy=(10**0.4, 1.4),  xycoords='data',
