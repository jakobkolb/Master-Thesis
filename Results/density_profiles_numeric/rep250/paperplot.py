...
print densdata[frames+1][-l,1]
print densdata[frames+1][-5:-1,1]
...
l1 = ax2.plot(densdata[frames+1][:,0],densdata[frames+1][:,1]/densdata[frames+1][-l,1]     ,zorder = 4, color = reds[j], label = r'$\rho^{BD}_0(r)$')
ax2.fill_between(plt[:,0],plt[:,1]/plt[-l,1]+2*plt[:,2]/plt[-l,1],plt[:,1]/plt[-l,1]-2*plt[:,2]/plt[-l,1],zorder = 2, color='grey', alpha = 0.4)
l2 = ax2.plot(densdata[frames+1][:,0],densdata[frames+1][:,3]/densdata[frames+1][-l,3]     ,zorder = 3, color = blues[j], label = r'$\rho^{BD}_1(r)$')
ax2.fill_between(plt[:,0],plt[:,3]/plt[-l,3]+2*plt[:,4]/plt[-l,3],plt[:,3]/plt[-l,3]-2*plt[:,4]/plt[-l,3],zorder = 1, color='grey', alpha = 0.4)
l3 = ax2.plot(analytic[:,0],analytic[:,2]/analytic[-m,2], '--', color = reds[k], zorder = 5, label = r'$\rho^{MOL}_0(r)$')
l4 = ax2.plot(analytic[:,0],analytic[:,1]/analytic[-m,1], '--', color = blues[k], zorder = 6, label = r'$\rho^{MOL}_1(r)$')
l5 = ax2b.plot(plt[:,0],plt[:,5], '--', color='grey', zorder = 0, label = '$U_1(r)$')

ax2.set_ylabel(r'$\rho_n(r)$')
ax2b.set_ylabel(r'$U_1(r)$')
