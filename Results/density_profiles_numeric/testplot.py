...
ax2b = ax2.twinx()
l1 = ax2.plot(densdata[frames+1][:,0],densdata[frames+1][:,1]     ,zorder = 4, color = reds[j], label = r'$\rho_1(r)$')
ax2.fill_between(plt[:,0],plt[:,1]+2*plt[:,2],plt[:,1]-2*plt[:,2],zorder = 2, color='grey', alpha = 0.4)
l2 = ax2.plot(densdata[frames+1][:,0],densdata[frames+1][:,3]     ,zorder = 3, color = blues[j], label = r'$\rho_2(r)$')
ax2.fill_between(plt[:,0],plt[:,3]+2*plt[:,4],plt[:,3]-2*plt[:,4],zorder = 1, color='grey', alpha = 0.4)
ax2.plot(analytic[:,0],2*np.mean(densdata[frames+1][-10:-2,1])*analytic[:,1], 'k--', zorder = 5)
l3 = ax2.plot(analytic[:,0],2*np.mean(densdata[frames+1][-10:-2,3])*analytic[:,2], 'k--', zorder = 6, label = r'$\rho_a(r)$')
l4 = ax2b.plot(plt[:,0],plt[:,5], '--', color='grey', zorder = 0, label = '$U_1(r)$')
