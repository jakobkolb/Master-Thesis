...
for i, f in enumerate(folders):
    if (f=='rnp'): break
    ax1.plot(Rvals, DeffLong[f] ,zorder = 4, color = blues[i], label = legendtxt[i])
...
for i, f in enumerate(folders):
    if (f=='rnp'): break
    ax2.plot(Rvals, DeffShort[f] ,zorder = 4, color = blues[i], label = legendtxt[i])
