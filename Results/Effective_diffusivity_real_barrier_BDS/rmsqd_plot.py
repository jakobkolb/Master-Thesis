...
for i, f in enumerate(folders):
    ...
    EDeffShort[i] = np.zeros((nbins))
    DeffLong[i] = np.zeros((nbins))
    EDeffLong[i] = np.zeros((nbins))

    for ibin in range(bcut,nbins-tcut):
        par, cov = cp.curve_fit(powerlaw, times[i0:i1], data[i][i0:i1,ibin], sigma =  data[i][i0:i1,ibin], absolute_sigma=True)
        DeffShort[i][ibin] = par
        EDeffShort[i][ibin] = np.sqrt(cov)
...
l1 = ax1.plot(Rvals, DeffLong[0] ,zorder = 4, color = reds[0], label = r'$r_d = 0.25$')
l2 = ax1.plot(Rvals, DeffLong[1] ,zorder = 4, color = reds[1], label = r'$r_d = 2.5$')
l3 = ax1.plot(Rvals, DeffLong[2] ,zorder = 4, color = reds[2], label = r'$r_d = 250$')
