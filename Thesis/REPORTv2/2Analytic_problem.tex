\section{Short summary of Stochastic Processes}
...
\end{equation}
where $\mathbb{A}$ denotes the transformation $\mathbb{T}\mathbb{D}$ and use the fact that the potential term vanishes everywhere but at the jump discontinuities. Therefore the equation reads
\begin{equation}
...
\begin{align}
    ...
    \tilde{\rho}_{i \ne 1}^{(j)}(r) &= c_{i,1}^{j} \exp\left[-r\sqrt{\frac{\lambda_i}{D}}\right] + c_{i,2}^{j} \exp\left[r\sqrt{\frac{\lambda_i}{D}}\right] 
    \label{fp_ind_sol}
\end{align}
...
\begin{equation}
    \tilde{\vect{\rho}}^{(j)} = \underbrace{ \left( \begin{array}{cllllllll}
       ...
       0   & 0             & e^{-r \alpha_2}   & e^{r \alpha_2 }   & 0              & 0             & 0 & \cdots &\\
       0   & 0             & 0                 & 0                 & e^{-r\alpha_3} & e^{r\alpha_3} & 0 & \cdots &\\
       \vdots  &&&&&&&\ddots &\\
       \vdots  &&&&&&&&\ddots
...
\begin{equation}
    ...
    \label{lgs}
\end{equation}
where dashes denote derivatives with respect to $r$. Solving this system of linear equations yields the somehow lengthy expressions for the coefficients $c_{i,k}^{(j)}$. \\ We obtain the actual density profile by applying the inverse transformation $\mathbb{A}^{-1}$
...
\begin{align}
    ...
    &= 4 \pi D R_s^{2} \sum_{n=1}^{N} \left\{ \mathbb{A}^{-1} \left. \frac{\partial}{ \partial r}\right|_{R_s} \tilde{\vect{\rho}} \right\}_n
    \label{Rate}
\end{align}
