...
\begin{equation}
    [W_{1,j}, \cdots , W_{j-1,j},1 - \sum_{i \ne j} W_{i,j}, W_{j+1,j}, \cdots , W_{N,j}]
    \label{num_meq}
\end{equation}
it hits. The particle does not change its state, if the random number is in bin number $j$ and changes its state to $n = k$ if the random number hits bin number $k$.\\
...
The fit conditions at the jump discontinuities of the potential barrier as given in \eqref{dens_fit} are implemented as follows. If a particle in state $n$ crosses the boundary of the potential barrier from a lower to a higher level, it has a probability $P_r = P_{\Delta U_n}$ to be reflected and a probability $P_p = 1 - P_{\Delta U_n}$ to pass where $P_{\Delta U_n}$ is given by an Arrhenius factor
\begin{equation}
    P_{ \Delta U_n} + \exp \left[\frac{\Delta U_n}{K_B T}  \right].
