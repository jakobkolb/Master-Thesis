...
\end{equation}
The Matrix $\hat{U}$ has diagonal elements $U_{ii}$ equal to the different hight of the potential barrier and is zero elsewhere. For the rate matrix holds: $W_{ii} = -\sum_{i \ne j} W_{ij}$. The Matrix $\hat{W}$ is generally not symmetric. But one can show that there always is an eigenvector to it, that satisfies $\hat{W} \vect{\rho}_{eq} = 0$ and that for this eigenvector holds the following relation, also known as {\it detailed balance}:
\begin{equation}
