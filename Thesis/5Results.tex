...
\comment{On intermediate summaries: is it better to have one at the end of each section, at the end of a couple of sections that belong together somehow, or only one extensive summary at the very end?}
\begin{equation}
...
\comment{Please give some comments on this section since I am not sure, whether it hits the point of what is meant when speaking about ``mapping on a Non-Markovian Description''. I figured, it would be clear from the discussion in Section \ref{Numeric_Study} that the approach of ``rates over a potential of mean force'' is bound to fail when timescale separation between barrier fluctuations and diffusion relaxation breaks down. Does this point need to be stressed in an extra section? The ``addition of rates thing is interesting though, don't you think?}
...
\comment{Does this make sense? I thought so..} 
\begin{equation}
